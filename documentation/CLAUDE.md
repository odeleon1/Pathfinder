...
- **`/dev/video0` is not stable — check the device node before blaming anything else.** The BRIO re-enumerated to `/dev/video1`–`video4` after being moved to a different USB port, and the launch file's default `/dev/video0` no longer existed. The only symptom was an empty viewer; the actual error (`Failed opening device /dev/video0: No such file or directory`) was buried in the camera node's log. `ls /dev/video*` first — the lowest-numbered BRIO node is the RGB stream. Pass `camera_device:=/dev/videoN`.
- **The fan-out limit is per-topic subscriber count, not message size.** This is the corrected root cause; earlier notes blamed size alone. A large topic serves its *first* subscriber at full rate and starves every additional one. Measured on `/image_raw` (1.22 MB) with the depth node and rgbd_sync already attached, a third subscriber got **0.20 Hz**. The same data on a dedicated topic with one subscriber runs at **24 Hz**. Two independent workarounds, both verified:
  - **Compress it.** `/image_raw/compressed` is 107 KB vs 1220 KB and delivers **24.40 Hz** to an additional subscriber where raw gives 0.20 Hz — a 122× difference. Requires `ros-jazzy-compressed-image-transport` (installed 2026-07-27).
  - **Give it its own topic.** `ros2 run image_transport republish --ros-args -p in_transport:=compressed -p out_transport:=raw -r in/compressed:=/image_raw/compressed -r out:=/viz/image_raw` → 24.13 Hz on `/viz/image_raw`.
- **Anything published from inside the container barely escapes it — except `/tf`.** Measured externally while the pipeline ran: `/tf` **17.7 Hz** (fine), `/odom` **0.13 Hz** (starved). `/odom` is a few hundred bytes, so this is not a size effect — it is the intra-process publisher. Nothing outside the container currently needs `/odom`, so it costs nothing today. **It will matter in Phase 2**, when SPOT's odometry crosses that same boundary. Understand it before then.
- **The viewer's camera/depth panels do not work, and map-only is the supported mode.** `rtabmap_viz` approx-syncs `/odom`, `/image_raw`, `/depth/image`, `/camera_info`. Feeding it images *is* solvable (republish to a dedicated topic, above — starvation goes to 0 and it renders at ~13% CPU), but it then aborts every frame with `Could not get odometry pose from TF`. Unresolved: `odom -> camera` is present on `/tf` at 17.7 Hz and `odom_frame_id:=odom` was set explicitly. The remaining suspect is the starved `/odom`, and chasing it means either unwinding the composition that bought 7→27 Hz or digging into rtabmap's TF buffer internals — a bad trade for a preview panel. Map-only runs clean (0 starvation, 0 aborts, verified over 90 s):
  ```bash
  ...
  ```
  You keep the 3D point cloud and the graph. To watch the camera feed as well, run `rqt_image_view` on `/image_raw/compressed` in a third terminal — its own topic, its own subscriber, full rate, no cost to the pipeline.
